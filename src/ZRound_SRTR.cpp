...
const int MAX_TAGS = 10; // maximum tags allowed in a single race.
const int MIN_LAP_TIME = 1000;   // The reader is too fast. Use this to prevent double reads.
...
TagRecord TagsSeen[MAX_TAGS]; // where we store transponders seen during race
...
void addTagToList(byte EPC_Seen[], unsigned long timeSeen)
{
  ...
  if (!added)
  {
    ...
    for (int i = 0; i < MAX_TAGS && added == false; i++)
    {
...
bool hasBeenSeenRecently(byte EPC_Seen[], unsigned long timeSeen)
{
  ...
  for (int i = 0; i < MAX_TAGS; i++)
  {
    if ((timeSeen - TagsSeen[i].timeLastSeen) < MIN_LAP_TIME &&
...
void printTagsSeen()
{
  Serial.println("debug_array:");
  for (int i = 0; i < MAX_TAGS; i++)
  {
    for (int j = 0; j < EPC_BYTES; j++)
    {
      if (TagsSeen[i].tagEPC[j] < 0x10)
        Serial.print(0);
